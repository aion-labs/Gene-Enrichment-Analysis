...
def main():
    ...
    example_gene_list = Path("data/gene_lists/hypoxia-genes.symbols.txt")
    if not example_gene_list.exists():
        print("❌ Error: Example gene list not found")
        print("   Expected: data/gene_lists/hypoxia-genes.symbols.txt")
        sys.exit(1)
    ...
    success, output = run_command([
        "python", "code/cli.py",
        "--genelist", str(example_gene_list),
    ...
    if success:
        # Check output files
        output_dir = Path("cli_results/hypoxia-genes")
        expected_files = [
    ...
    if success:
        output_dir = Path("test_regular_custom/hypoxia-genes")
        expected_files = [
            "combined_regular_results.tsv",
            "regular_enrichment_snapshot.json"
        ]
    ...
    if success:
        output_dir = Path("test_iterative/hypoxia-genes")
        expected_files = [
    ...
    # Note: output directory will use the stem of the file name (hypoxia-genes)
    
    success, output = run_command([
        "python", "code/cli.py",
        "--mode", "regular",
        "--genelist", str(example_gene_list), str(test_gene_list2),
        "--background", background_file,
    ...
    if success:
        ...
        for gene_list_name in ["hypoxia-genes", "test_gene_list2"]:
