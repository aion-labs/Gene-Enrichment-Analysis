...
Storage Strategy:
    - Permutation files are saved directly to /results/permutation_results/ (persistent storage)
    - All columns are saved to match format of previous runs (50-500):
      * Library, Iteration, Term, Description
      * iteration p-value, iteration -log(p-value)
    ...
    - Individual iteration JSON/TSV files are not saved (disabled for permutation generation)

Usage:
    python tools/generate_permutation_distribution.py [--n-permutations N] [--n-jobs N] [--resume]
    ...
    Example for c6i.8xlarge (32 vCPUs):
        python tools/generate_permutation_distribution.py --n-jobs 32
...
# When script is in tools/ folder, go up one level to project root
...
# Script is now in tools/, so need to import from code/
...
sandbox_dir.mkdir(exist_ok=True)  # Ensure sandbox directory exists
file_handler = logging.FileHandler(str(sandbox_dir / "permutation_distribution.log"))
...
def run_single_permutation(
    ...
    try:
        ...
        if all_results:
            ...
            # Keep all columns for consistency with previous runs
            # Previous runs (50-500) included all columns, so we keep all here too
            # This includes: Description, -log(p-value), Full list data, etc.
            # Note: If file size becomes an issue, can filter later, but for now
            # we keep all columns to match the format of the 50-500 run
            logger.debug(f"Keeping all {len(combined_df.columns)} columns: {list(combined_df.columns)}")
...
def run_permutations_for_size(
    ...
    Args:
        output_dir: Directory in /results for permutation_results
